...
export default class ListPage {
	...
	constructor(config: ListPageConfig) {
		...
		let processFn = config.process
		let getList = config.getList
				...
				return _promiseApi(requestData, configData)
					.then(res => {
						if (res.statusCode === 200) {
							typeof processFn === 'function' && processFn.call(this, res)
							let list = isRemoveList ? [] : (table.list || [])
							let resList = typeof getList === 'function' ? getList.call(this, res.data.Result) : (res.data.Result[filedMaps.list] || [])
							let resData = this._processFn(resList).map((item: any) => {
