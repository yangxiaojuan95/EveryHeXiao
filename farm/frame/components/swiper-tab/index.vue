<template>
	<view class='tabs flex c-center' :style="{backgroundColor: background, fontSize: tabFontSize}">
		<view class="tabs-item fg1" :class="{'active' : currentTab === index}" v-for="(item, index) in tabs"
			:key="index" @click="onSetTab(index)">
			<text :id="'tab' + index"
				:style="{color: currentTab === index ? activeColor : color}">{{ item.text }}</text>
		</view>
		<view v-if="isShow" class="active-bar"
			:style="{left: activeBar.left, right: activeBar.right, backgroundColor: barColor, height: upxTopx(barHeight)}">
		</view>
	</view>
</template>
		...
		methods: {
			...
			setActiveBar: function(direction) {
				...
				query.select(`#tab${this.currentTab}`).boundingClientRect(rect => {
					...
					let right = this.$store.state.system.systemInfo.windowWidth - uni.upx2px(this.right) - rect
						.right - this.overwidth + 'px'
					if (direction) {
						if (direction === 'left') {
