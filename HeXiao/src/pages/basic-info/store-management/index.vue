  ...
  table: {
    ...
    els: [
      ...
      },
      {
        ...
        renderFn(row) {
          return <el-image width='100px' height='100px' src={`${pathUrl}${row.Image}`}></el-image>
        }
      },
      {
        prop: "Lng",
...
const handleScanningCode = async (row: any) =>{
  const result = await processdRequest.get('/api/BackShop/ShopGenerateQrCode', {userid:row.ShopID})
  image.value = result
  visible.value = true
}
