  ...
  table: {
    ...
    els: [
      ...
      },
      {
        ...
        renderFn(row) {
          return (
            <el-tag type={row.IsEnable ? 'success' : 'error'}>
              {row.IsEnable ? '启用' : '禁用'}
            </el-tag>
          )
        },
  ...
  form: {
    ...
    async bindData(formData) {
      if (!formData.GoodsCode) {
        formData.GoodsCode = await genCode({
          type: '商品',
        })
      }
      // if (formData.GoodsID) {
    ...
    async beforeSubmit(formData) {
      formData.Code = formData.GoodsCode
      console.log(formData.ImageList, 'formData.ImageList')
      // formData.ImageList =
      //   formData.ImageList?.split(',').map((item: any) => {
      //     return {
      //       Image: item,
      //     }
      //   }) || []
...
const handleDelete = async () => {
  ...
  await processdRequest.post('/api/BackGoods/DelGoods', {
